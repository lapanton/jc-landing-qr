...
export const PendantsTalisman = (props) => {
  ...
  const buyPendant = () => {
    const arr1 = [
      {
        product: "pendantTalisman",
        stone: quasarStone,
        price: +priceData["talisman-kulon-genskii"],
      },
    ];
    const checkedItem = card.filter(
      (value) => value.product !== "pendantTalisman",
    );
      ...
      <Inner>
        ...
        <ChooseStone>
          ...
          <p className="description">
            ...
            {quasarStone === "12p-t" &&
              (langProps.locale === "ru"
                ? "Кошачий глаз - минерал защищает от бед и болезней"
        ...
        <PriceBuySection>
          ...
          <WrapPrices>
            <p className="price">
              {priceData["talisman-kulon-genskii"]}
              <span> ₽</span>
            </p>
          ...
          {showDescr && (
            <DescriptionView>
              <p className="slave-text">
                {langProps.locale === "ru"
                  ? "Кулон Jewel Cocktail в стиле минимализм. Со сменными вставками из натуральных материалов. Особенная вставка с QR ссылкой на персональный талисман."
                  : "Jewel Cocktail talisman-pendant in minimalist style with replaceable natural stones. Special stone with QR link to your personal talisman"}
              </p>
